<template>
    <div>
    <h3>
        Open Crypto Fate / Baccarat
    </h3>

    <textarea spellcheck="false"
              name="public-key"
              class="public-key"
              ref="baccaratVerifyPublicKey"
    >-----BEGIN PUBLIC KEY----------END PUBLIC KEY-----</textarea>

    <textarea name="other-datas"
              class="other-datas"
              placeholder="Paste the datas copied from Baccarat verify page here."
              ref="baccaratVerifyDatas"
              spellcheck="false"
    >
                </textarea>

    <div class="verify-part">
        <button class="verify-btn"
                @click="clickToVerify"
        >
            Verify</button>
        <img class="github-logo" src="../assets/githublogo.png" alt=""><a href="https://github.com/QuarkChain/open-cryptofate/tree/master/GameVerify/src/components/Baccarat.js" target="_blank">Baccarat.js</a>
    </div>

    <ul class="verify-result baccarat-result"
        ref="baccaratVerifyResult">
    </ul>
    </div>
</template>


<script src="./Baccarat.js"></script>

<style scoped>

</style>
